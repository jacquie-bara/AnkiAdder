...
(async () => {
  ...
  const app = await _electron.launch({ executablePath: path.resolve('release/win-unpacked/AnkiAdder.exe'), args: ['--smoke-test', '--user-data-dir=' + directory], env });
  try {
    const page = await app.firstWindow();
    await page.getByRole('heading', { name: 'One word. A whole new world.' }).waitFor();
    ...
    console.log('Packaged Windows app starts successfully; compact defaults, audio bridge, UI and settings verified.');
